 ...
 */
async function publishJobEvent(redisPublisher, payload) {
    try {
        await redisPublisher.publish('job-events', JSON.stringify(payload));
    }
    catch (err) {
        // Non-fatal — real-time updates are best-effort
        (0, logger_1.createChildLogger)({ jobId: payload.jobId }).warn('Failed to publish job event to Redis', { error: err });
    }
}
/**
...
function createTimeoutPromise(ms) {
    return new Promise((_, reject) => setTimeout(() => reject(new Error(`Job execution timed out after ${ms}ms`)), ms));
}
 ...
 */
async function executeJobHandler(type, input) {
 ...
 */
function createProcessor(queueName, workerId, prismaClient, redisPublisher, onJobStart, onJobEnd) {
    const baseLog = (0, logger_1.createChildLogger)({ workerId, queueName });
    baseLog.info(`Creating BullMQ Worker for queue "${queueName}"`);
    // ── BullMQ Processor Function ──────────────────────────────────────────────
    const processorFn = async (bullJob) => {
        ...
        const log = (0, logger_1.createChildLogger)({ workerId, queueName, jobId, bullJobId: bullJob.id });
        ...
        try {
            ...
            const isTimeout = execErr instanceof Error &&
            ...
            log[isTimeout ? 'warn' : 'error'](isTimeout ? 'Job timed out' : 'Job execution failed', { error: execErr, durationMs });
            ...
            if (timedOut) {
                throw new bullmq_1.UnrecoverableError(`Job ${jobId} timed out after ${timeoutMs}ms`);
            }
    ...
    const worker = new bullmq_1.Worker(queueName, processorFn, {
        connection: (0, redis_1.createBullMQRedisClient)(),
